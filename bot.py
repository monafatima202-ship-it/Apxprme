...
# ====================== DATABASE ======================
def init_db():
    conn = sqlite3.connect('apx_v140.db')
...
async def show_dashboard(m_c):
    ...
    if is_vip:
        ...
        kb.row(types.InlineKeyboardButton(text="🔑 GET ACCESS KEY", callback_data="get_key"))
    else:
    ...
    if is_regen and data.get("last_report"):
        ...
        while curr < end:
            for p in data["pairs"]:
                ...
                dir_icon = "⇨"
                trade = random.choice(["PUT ", "BUY "])
                body += f"⧉ {p_clean:10} - {curr.strftime('%H:%M')} {dir_icon} {trade}\n"
            curr += datetime.timedelta(minutes=random.randint(8, 14))
        
        footer = (
            "\nRULES ‼️\n"
            "- DO NOT TRADE IN MARKET LESS THAN 80%\n"
...
async def regen_sig(callback: types.CallbackQuery):
    await callback.answer("Displaying cached data..."); await execute_signals(callback, is_regen=True)
